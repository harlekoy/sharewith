...
'providers' => [
    // ...
    Harlekoy\ShareWith\PermissionServiceProvider::class,
];
```
...
```bash
php artisan vendor:publish --provider="Harlekoy\ShareWith\PermissionServiceProvider" --tag="migrations"
```
...
```bash
php artisan vendor:publish --provider="Harlekoy\ShareWith\PermissionServiceProvider" --tag="config"
```
...
return [
    ...
    'models' => [
         ...
         * `Harlekoy\ShareWith\Contracts\Permission` contract.
         */

        'permission' => Harlekoy\ShareWith\Models\Permission::class,
         ...
         * `Harlekoy\ShareWith\Contracts\Role` contract.
         */

        'role' => Harlekoy\ShareWith\Models\Role::class,
...
$app->routeMiddleware([
    ...
    'permission' => Harlekoy\ShareWith\Middlewares\PermissionMiddleware::class,
    'role'       => Harlekoy\ShareWith\Middlewares\RoleMiddleware::class,
]);
```
...
$app->register(Harlekoy\ShareWith\PermissionServiceProvider::class);
```

## Usage

First, add the `Harlekoy\ShareWith\Traits\HasRoles` trait to your `User` model(s):

```php
use Illuminate\Foundation\Auth\User as Authenticatable;
use Harlekoy\ShareWith\Traits\HasRoles;
...
```php
use Harlekoy\ShareWith\Models\Role;
use Harlekoy\ShareWith\Models\Permission;
...
The `role` scope can accept a string, a `\Harlekoy\ShareWith\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\Harlekoy\ShareWith\Models\Permission` object or an `\Illuminate\Support\Collection` object.
...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`  and `removeRole` functions can accept a
 string, a `\Harlekoy\ShareWith\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `Harlekoy\ShareWith\Models\Permission` object.
...
All these responses are collections of `Harlekoy\ShareWith\Models\Permission` objects.
...
{
    if ($exception instanceof \Harlekoy\ShareWith\Exceptions\UnauthorizedException) {
        // Code here ...
    }
...
```php
    public function setUp()
    {
        ...
        $this->app->make(\Harlekoy\ShareWith\PermissionRegistrar::class)->registerPermissions();
    }
```
...
- Your `Role` model needs to implement the `Harlekoy\ShareWith\Contracts\Role` contract
- Your `Permission` model needs to implement the `Harlekoy\ShareWith\Contracts\Permission` contract
